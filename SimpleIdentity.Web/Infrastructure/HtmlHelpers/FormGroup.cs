namespace SimpleIdentity.Web.Infrastructure.HtmlHelpers
{
    ...
    public static class FormGroup
    {
        ...
        private const string FullWidth = "col-md-12";
        ...
        public static MvcHtmlString FormGroupFor<TModel, TValue>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TValue>> expression, bool useInlineValidation)
        {
            var input = htmlHelper.EditorFor(expression, new { htmlAttributes = new { @class = InputClass } });
            var generatedGroup = GenerateGroup(htmlHelper, expression, useInlineValidation, input);

            return new MvcHtmlString(generatedGroup.ToString());
        }

        public static MvcHtmlString GenerateGroup<TModel, TValue>(
            HtmlHelper<TModel> htmlHelper,
            Expression<Func<TModel, TValue>> expression,
            bool useInlineValidation,
            MvcHtmlString input)
        {
        ...
        public static MvcHtmlString GenerateButtonGroup(HtmlHelper htmlHelper, MvcHtmlString input)
        {
            var outerDiv = GenerateOuterDiv();
            var innerDiv = GenerateInnerDivWithFullWidth();
        ...
        private static TagBuilder GenerateInnerDivWithFullWidth()
        {
            var innerDiv = new TagBuilder("div");
            innerDiv.AddCssClass(FullWidth);
            return innerDiv;
        }
    }
}